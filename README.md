...
#### **접근 제어자의 종류엔 어떤게 있는지 설명하시오.**   
    다섯가지 접근레벨   
        open(클래스, 클래스 맴버에만 사용가능): 모듈의 모든 소스 파일 내에서 사용가능, 정의한 모듈을 가져오는 다른 모듈의 소스파일에서도 사용할 수 있다.
        public: 모듈의 모든 소스 파일 내에서 사용가능, 정의한 모듈을 가져오는 다른 모듈의 소스파일에서도 사용할 수 있다.
    open과 public의 차이점   
        public 접근권한이 있는 클래스, 클래스 맴버는, 정의된 모듈 내에서만 오버라이딩, 서브클래싱 가능!!
        open은 상관없이 오버라이딩, 서브클래싱 가능
        
        internal: 모듈의 모든 소스 파일 내에서 사용가능, 그러나 모듈 외부의 소스 파일에서는 사용할 수 없다.
        File-private: 정의한 소스 파일 내에서만 사용이 가능하다.
        private: 선언한 곳이나, 동일한 파일의 extension에서 사용이 가능하다.
